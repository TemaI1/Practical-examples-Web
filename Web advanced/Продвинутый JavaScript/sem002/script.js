...
const num2 = document.querySelector('.user-input');
const button2 = document.querySelector('.add-button');
const list2 = document.querySelector('.item-list');
const message2 = document.querySelector('.error-message');
button2.addEventListener('click', addElement);

function addElement() {
    try {
        if (num2.value.length >= 3 && num2.value.length <= 10) {
            list2.innerHTML += `<li>${num2.value}</li>`;
        } else {
